...
- **Google Maps Android key:** Injected at EAS prebuild via a local Expo config plugin at `plugins/with-google-maps-key.js` (registered in `app.json` `expo.plugins`). The plugin reads `GOOGLE_MAPS_API_KEY` from the build environment and adds `<meta-data android:name="com.google.android.geo.API_KEY">` to AndroidManifest. For EAS Build to have access to it, `GOOGLE_MAPS_API_KEY` must be set as an EAS Project Secret (expo.dev → project → Secrets) — Replit secrets do NOT auto-forward to EAS Build. If the env var is missing, the plugin warns and the build still completes (maps just won't render on Android). The `android.config.googleMaps.apiKey: ""` placeholder in `app.json` is unused but harmless. iOS / Web do not require this key. Do NOT use `eas.json` `prebuildCommand` for this — it appends args to `expo prebuild`, not arbitrary shell commands.
